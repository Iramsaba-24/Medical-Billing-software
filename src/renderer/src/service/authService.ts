...
//validation api
export const checkBusinessFieldExists = async (
  field: "gstin" | "licenseNumber",
  value: string
): Promise<boolean> => {
  try {
    const res = await axios.get(
      `${API_BASE_URL}/BusinessDetail`,
    );

    const list = res.data as CreateBusinessDetailRequest[];

    return list.some((item) => {
      const fieldValue =
        field === "gstin" ? item.gstin : item.licenseNumber;
