...
//     items: finalItems,
//   });
// }, [invoiceNo, reset]); // inventoryOptions हटवले
 
// // Items array sync useEffect हटवा किंवा comment करा
// // useEffect(() => {
// //   watchedItems?.forEach((item, index) => {
// //     const selectedItem = inventoryOptions.find(
// //       (i) => i.value === item.item
...
const EditInvoice = () => {
  ...
  useEffect(() => {
    ...
    const finalItems =
      items.length > 0 ? items : [{ item: "", qty: 1, price: 0 }];

    reset({
      name: foundInvoice.name,
      date: dayjs(foundInvoice.date, "DD/MM/YYYY", true), // ✅ FIXED
      status: foundInvoice.status,
      items: finalItems,
    });
  }, [invoiceNo, reset]);
