    ...
    const fetchInvoice = async () => {
      try {
        ...
        const originalSubtotal = items.reduce(
          (sum: number, i: RetailInvoiceItemResponse) =>
    ...
    try {
      ...
      if (newTotal < originalTotal) {
        ...
        const originalDiscount = data.items.reduce(
          (sum: number, item: InvoiceItem) => sum + (item.discount || 0),
          0,
        );
            ...
            >
              <DropdownField
                ...
                onChangeCallback={(val: string) => {
                  ...
                  const otherSubtotal = allItems.reduce(
                    (sum: number, it: InvoiceItem, i: number) => {
                  ...
                  if (grandSubtotal < invoiceOriginalTotal) {
                    ...
                    const afterMedipoints = grandSubtotal - medipoints;
                    const gstAmount =
                      (afterMedipoints * invoiceGstPercent) / 100;
                    const grandFinal = afterMedipoints + gstAmount;
                    const ratio = thisSubtotal / grandSubtotal;
                    displayPrice = grandFinal * ratio;
                  } else {
                    displayPrice =
                      thisSubtotal + (thisSubtotal * invoiceGstPercent) / 100;
                  }
              ...
              <TextInputField
                ...
                rules={{
                  ...
                  validate: (value: number) => {
                    const selectedItem = inventoryOptions.find(
                      (i) =>
                        i.value === methods.getValues(`items.${index}.item`),
                    );
