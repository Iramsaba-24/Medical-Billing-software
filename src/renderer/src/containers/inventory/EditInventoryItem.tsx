...
useEffect(() => {
    if (item && groupOptions.length > 0 && supplierOptions.length > 0) {
        methods.reset({
            ...item,
            groupId: item.groupId?.toString() as unknown as number,
            distributorId: item.distributorId?.toString() as unknown as number,
        });
    }
}, [item, groupOptions, supplierOptions]);
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    ...
    console.log("DISTRIBUTORS:", distributors); // ✅ correct

    const options: OptionType[] = distributors
        .filter((d) => d.status === "Active")
        .map((d) => ({
            label: d.companyName,
            value: d.distributorId?.toString() || "", // ✅ safe
        }));
    ...
    const selectedGroup = groupOptions.find(
        (g) => g.value === data.groupId.toString()
    );

    const selectedSupplier = supplierOptions.find(
        (s) => s.value === data.distributorId.toString()
    );
    ...
    const updated = tableData.map((i) =>
        i.medicineId === data.medicineId
            ? {
                  ...data,
                  medicineGroup: selectedGroup?.label || "",
