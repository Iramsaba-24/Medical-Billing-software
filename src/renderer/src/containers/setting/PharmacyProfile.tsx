...
//     try {
//       const userId = Number(localStorage.getItem("userId")); // or from auth

//       if (!userId) return;

//       const data = await pharmacySettingsService.getSettings(userId);

//       // Backend data  React Form set
//       reset({
...
//   //   localStorage.setItem("drugLicense", data.drugLicenseNumber);
...
//       logoUrl: previewImage ?? undefined, //  send logo URL if available
//     };

//   //const response = await pharmacySettingsService.saveSettings(payload);

//   await pharmacySettingsService.saveSettings(payload);
...
function PharmacyProfile() {
  ...
  const { clearData } = useAutoSave({
    storageKey: "pharmacy_profile_form",
    methods,
  });
