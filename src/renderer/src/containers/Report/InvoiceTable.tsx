...
const InvoiceTable = () => {
  ...
  // LOAD DATA FROM LOCAL STORAGE (FIXED KEY)
  useEffect(() => {
    const loadInvoices = () => {
      const stored = localStorage.getItem("currentInvoice"); // 🔥 FIX HERE
      const parsed: InvoiceData[] = stored ? JSON.parse(stored) : [];
      setInvoices(parsed);
    };
    ...
    // listen updates from BillingTable
    window.addEventListener("invoiceUpdated", loadInvoices);
  ...
  // FILTER LOGIC (FIXED DATE PARSING)
  const filteredData = useMemo(() => {
    return invoices.filter((inv) => {
      const matchesStatus =
        statusFilter === "All" || inv.status === statusFilter;

      //  FIX: dd/mm/yyyy → proper Date
  ...
  //  DELETE FUNCTION (SYNC WITH BillingTable)
  const handleDeleteSelected = (rowsToDelete: InvoiceData[]) => {
    const updatedInvoices = invoices.filter(
      (inv) =>
        !rowsToDelete.some((row) => row.invoice === inv.invoice)
