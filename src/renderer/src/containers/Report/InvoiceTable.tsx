...
import { Paper, Typography, Chip, MenuItem, Select, FormControl, Stack, Divider } from "@mui/material";
...
const InvoiceTable = () => {
  ...
  const columns: Column<InvoiceData>[] = [
    ...
    { key: "status", label: "Status", render: (row) => {
        ...
        return <Chip label={row.status} size="small" sx={{ backgroundColor: current.bg, color: current.color, fontWeight: "600", borderRadius: "4px" }} />;
      },
    },
  ];
    ...
    <Paper sx={{ p: 3, borderRadius: 2 }}>
      ...
      <Stack direction="row" justifyContent="flex-end" spacing={2} sx={{ mb: 2 }}>
        ...
        <FormControl size="small">
          <Select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} sx={{ minWidth: 130, borderRadius: "8px", fontSize: "0.85rem" }}>
        ...
        {/* Time Filter */}
        <FormControl size="small">
          <Select value={timeFilter} onChange={(e) => setTimeFilter(e.target.value)} sx={{ minWidth: 140, borderRadius: "8px", fontSize: "0.85rem" }}>
